...
namespace Options {
  ...
  addRootGeometryOptions(options_t& opt)
  {
    opt.add_options()("geo-root-filename",
                      ...
                      po::value<read_strings>()->multitoken()->default_value({}),
                      "Name identifier for negative sensitive objects, odered along the series.")
                      ("geo-root-cmodulenames",
                      po::value<read_strings>()->multitoken()->default_value({}),
                      "Name identifier for central sensitive objects, odered along the series.")
                      ("geo-root-pmodulenames",
                      po::value<read_strings>()->multitoken()->default_value({}),
                      "Name identifier for positive sensitive objects, odered along the series.");
  }
  ...
  readRootLayerBuilderConfigs(const variable_map_t& vm, 
                              std::shared_ptr<const Acts::LayerCreator> layerCreator)
  {
    std::vector<Acts::TGeoLayerBuilder::Config> detLayerConfigs;
    
    // general stuff
    read_strings subdetectors     = vm["geo-subdetectors"].template as<read_strings>();
    ...
    read_strings nlayernames      = vm["geo-root-nlayernames"].template as<read_strings>();
    read_strings clayernames      = vm["geo-root-clayernames"].template as<read_strings>();
    read_strings playernames      = vm["geo-root-playernames"].template as<read_strings>();
    read_strings nsensitivenames  = vm["geo-root-nmodulenames"].template as<read_strings>();
    read_strings csensitivenames  = vm["geo-root-cmodulenames"].template as<read_strings>();
    read_strings psensitivenames  = vm["geo-root-pmodulenames"].template as<read_strings>();
