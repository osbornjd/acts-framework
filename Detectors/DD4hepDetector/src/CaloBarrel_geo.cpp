...
static Ref_t
create_element(Detector& description, xml_h e, SensitiveDetector sens)
{
  xml_det_t  x_det    = e;
  ...
  for (xml_coll_t li(x_det, _U(volume)); li; ++li) {
    ...
    dd4hep::Volume layerVolume(
        layerName, layerShape, description.material(x_layer.materialStr()));
