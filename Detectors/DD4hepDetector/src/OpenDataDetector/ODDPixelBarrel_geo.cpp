...
create_element(Detector& lcdd, xml_h xml, SensitiveDetector sens)
{
  ...
  // Visualization
  staveAssembly.setVisAttributes(lcdd, x_stave.visStr());

  // The DetElement tree, layer inside barrel
  // DetElement staveElement(barrelDetector, layerName, layerNum);
  ...
  // Loop over the modules and place them
  for (unsigned int moduleNum = 0; moduleNum < nModules; ++moduleNum) {
    ...
    PlacedVolume placedModule = staveAssembly.placeVolume(
        moduleAssembly, Position(0., -ymin + moduleNum * ystep, 0.));
  ...
  if (x_stave.hasChild(_U(subtraction)) and x_stave.hasChild(_U(tube))) {
    ...
    foamVolume.setVisAttributes(lcdd, x_sub.visStr());
    ...
    // placedFoam.addPhysVolID("component", ++compNum);

    Tube coolingPipe(x_pipe.rmin(), x_pipe.rmax(), staveHlength + x_pipe.dz());
    // Create the substraction
    Volume pipeVolume(
        "CoolingPipe", coolingPipe, lcdd.material(x_pipe.materialStr()));
    pipeVolume.setVisAttributes(lcdd, x_pipe.visStr());
    ...
    // placedPipe.addPhysVolID("component", ++compNum);
    ...
    for (unsigned int modCable = 0; modCable < 0.5 * nModules; ++modCable) {
      ...
      for (int side = -1; side < 2; side += 2) {
        ...
        // Create the substraction
        Volume cableVolume("Cable", cable, lcdd.material(x_pipe.materialStr()));
        cableVolume.setVisAttributes(lcdd, x_pipe.visStr());
  ...
  for (xml_coll_t lay(xml, _U(layer)); lay; ++lay, ++layerNum) {
    ...
    string layerName = _toString((int)layerNum, "layer%d");
    // The Module envelope volume
    ...
    if (x_layer.hasChild(_U(support))) {
      ...
      // placedSupport.addPhysVolID("component", ++compNum);
    }
  ...
  placedBarrel.addPhysVolID("barrelendcap", 0);
