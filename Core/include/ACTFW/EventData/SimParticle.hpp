...
namespace FW {
  ...
  namespace Data {
      ...
      public:
               ...
               double deltaTime = 0.) 
        {
          ...
          if (m_p){ 
            ...
            m_timeStamp += deltaTime;
            if (m_pathInX0 >= m_limitInX0 
             || m_pathInL0 >= m_limitInL0 
    ...
    /// @note if a sampler changes one of the parameters, consistency
    /// can be broken, so it should update the rest (no checking done)
    template <typename particle_t> struct SimVertex {
      ...
      // Outgoing particles
      const std::vector<particle_t>& outgoing() const
      { 
        return out; 
      }
