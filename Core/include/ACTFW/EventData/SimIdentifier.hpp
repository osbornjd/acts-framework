...
namespace FW {
  ...
  class SimIdentifier
  {
    ...
    private:
      ...
      std::vector<const SimParticle*> m_truthParticles = {}; //!< the attached particles

    };
    
    inline SimIdentifier::SimIdentifier() : m_id(0) {}
    
    inline SimIdentifier::SimIdentifier(const SimIdentifier& other) : m_id(other.m_id){}
    ...
    inline SimIdentifier::SimIdentifier(identifier_type value, 
                                        std::vector<const SimParticle*> truthParticles) 
      : m_id(value)
      , m_truthParticles(std::move(truthParticles)) 
    {}
