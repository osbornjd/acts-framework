...
/// Proxy class for iterating over groups of elements within a container.
///
/// @note Each group will contain at least one element.
///
/// The proxy should always be used as part of a range-based for loop. In
/// combination with structured bindings to redue the boilerplate, the group
...
///     }
///
template <typename KeyGetter, typename Iterator>
class GroupBy
{
public:
  /// The key type that identifies elements within a group.
  using Key = std::decay_t<decltype(KeyGetter()(*Iterator()))>;
  /// A Group is an iterator range with the associated shared key.
...
private:
  ...
  findEndOfGroup(Iterator start) const
  {
    ...
    if (start == m_end) { return start; }
    // search the first element that does not shares a key with the start.
...
template <typename KeyGetter, typename Container>
GroupBy<KeyGetter, typename Container::const_iterator>
makeGroupBy(KeyGetter keyGetter, const Container& container)
{
