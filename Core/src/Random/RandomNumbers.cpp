...
FW::ProcessCode FW::RandomNumbers::setConfiguration(const FW::RandomNumbers::Config &cfg)
{
    ...
    m_uniform = UniformDist(m_cfg.uniform_parameters[0],m_cfg.uniform_parameters[1]);
...
double FW::RandomNumbers::draw(FW::Distribution dType) {
    ...
    switch (dType) {
        ...
        case Distribution::landau :
        {
            ...
            double res = m_cfg.landau_parameters[0] + landau_quantile(x, m_cfg.landau_parameters[1]);
            return res;
        }
    }
    return 0.;
