...
namespace Options {
  ...
  addParticleGunOptions(AOPT& opt){
    opt.add_options()
      ...
      ("pg-ptrange",
       po::value<read_range>()->default_value({100.*au::_MeV,1e5*au::_MeV}),
  ...
  readParticleGunConfig(const AMAP& vm){
    ...
    particleGunConfig.nParticles    
      = vm["pg-nparticles"].template as<size_t>();
    particleGunConfig.d0Range       = {{ d0r[0], d0r[1] }};
    particleGunConfig.z0Range       = {{ z0r[0], z0r[1] }};
    particleGunConfig.phiRange      = {{ phir[0], phir[1] }};
    particleGunConfig.etaRange      = {{ etar[0], etar[1] }};
    particleGunConfig.ptRange       = {{ ptr[0], ptr[1] }};
    particleGunConfig.mass          
      = vm["pg-mass"].template as<double>()*au::_MeV;
    particleGunConfig.charge        
      = vm["pg-charge"].template as<double>()*au::_e;
    particleGunConfig.randomCharge  
      = vm["pg-chargeflip"].template as<bool>();
    particleGunConfig.pID           
      = vm["pg-pdg"].template as<int>()*au::_MeV; 
