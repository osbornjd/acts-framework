...
FW::ProcessCode
FW::SpacePointFinder::clusterReading(AlgorithmContext& ctx, const DetData*& detData) const
{
	//Load hit data from Whiteboard
	if (ctx.eventStore.get(m_cfg.collectionIn, detData) != FW::ProcessCode::SUCCESS)
	{
...
FW::SpacePointFinder::localCoords(const Acts::PlanarModuleCluster& hit) const
{
	...
	Acts::Vector2D local(par[Acts::ParDef::eLOC_0],
						par[Acts::ParDef::eLOC_1]);
	return local;
}
...
const double 
FW::SpacePointFinder::differenceOfHits(const Acts::PlanarModuleCluster& hit1, const Acts::PlanarModuleCluster& hit2) const
{
	...
	theta1 = M_PI / 2 - atan(pos1.z() - m_cfg.vertex.z() 
			/ sqrt((pos1.x() - m_cfg.vertex.x()) * pos1.x() - m_cfg.vertex.x())
			+ (pos1.y() - m_cfg.vertex.x()) * (pos1.y() - m_cfg.vertex.x()));
	phi2 = atan2(pos2.y() - m_cfg.vertex.y(), pos2.x() - m_cfg.vertex.x());
	theta2 = M_PI / 2 - atan(pos2.z() - m_cfg.vertex.z() 
			/ sqrt((pos2.x() - m_cfg.vertex.x()) * pos2.x() - m_cfg.vertex.x())
	...
	{
		ACTS_DEBUG("Squared theta angle " << diffTheta2 << " too large - points are not combined");
		return -1.;
	}
	...
	{
		ACTS_DEBUG("Squared phi angle " << diffPhi2 << " too large - points are not combined");
		return -1.;
	}
...
FW::SpacePointFinder::combineHits(const std::vector<Acts::PlanarModuleCluster>& vec1, const std::vector<Acts::PlanarModuleCluster>& vec2) const
{
...
void
FW::SpacePointFinder::findOverlappingClusters(const DetData*& detData, std::vector<std::vector<FW::SpacePointFinder::CombinedHits>>& allCombHits) const
{
	//Declare temporary storage
	std::vector<Acts::PlanarModuleCluster> const * module1;
	std::vector<Acts::PlanarModuleCluster> const * module2; //TODO: Annahme ist hier, dass es nur 2 surfaces pro layer gibt
	...
	for(auto& volumeData : *detData)
		for(auto& layerData : volumeData.second)
		{
			...
			if(module1 && module2)
			{
				std::vector<FW::SpacePointFinder::CombinedHits> combHits = combineHits(*module1, *module2);
...
void
FW::SpacePointFinder::filterCombinations(std::vector<std::vector<FW::SpacePointFinder::CombinedHits>>& allCombHits) const
{	
	...
	for(auto& layer : allCombHits)
		...
		for(auto& combination : layer)
		{
			for(auto& combinationCompare : layer)
				...
				//Hits on the first module are different from each other by construction but a hit on the second module can appear in multiple combinations.
				if((combination.hitModule1 != combinationCompare.hitModule1) && (combination.hitModule2 == combinationCompare.hitModule2))
					ACTS_INFO("Warning: Multiple candidate selection not implemented yet");
		}
}
...
FW::SpacePointFinder::endsOfStrip(const Acts::PlanarModuleCluster& hit) const
{
	...
	auto& boundariesY = binData[1].boundaries(); //TODO: Es sollte erfahren werden, wo die lange und die kurze seite des strips ist
	...
	Acts::Vector2D topLocal = {(boundariesX[binX] + boundariesX[binX + 1]) / 2,
								boundariesY[binY + 1]};
	Acts::Vector2D bottomLocal = {(boundariesX[binX] + boundariesX[binX + 1]) / 2,
								boundariesY[binY]};
...
FW::SpacePointFinder::calculateSpacePoints(std::vector<std::vector<CombinedHits>>& allCombHits) const
{
...
/// resulting point is a point x on the first surfaces. This point is along 
...
/// As a consequence of this demand y = k * x needs to be on the connecting 
...
/// equation leads to the condition for m as given in the following algorithm
...
{
  //TODO:Check, ob es sich bei den Daten um strip Aufnahmen handelt. Vllt via Abstaende oder Segmentation
  //TODO:Die Richtung aus der das Teilchen kommt ist fixed auf m_cfg.vertex. Sollte vllt modifizierbar sein
  ACTS_DEBUG("::execute() called for event " << ctx.eventNumber);
  
  //DetData is typename of DetectorData<geo_id_value, Acts::PlanarModuleCluster>
  ...
  //Write to Whiteboard
  if (ctx.eventStore.add(m_cfg.collectionOut, std::move(spacePoints)) //TODO: was macht add?
      != ProcessCode::SUCCESS)
