  ...
  for (auto& volumeData : *detData)
    for (auto& layerData : volumeData.second) {
      ...
      for (auto& moduleData : layerData.second) {
		...
		if(moduleData.first + 1 >= modules.size())
			modules.resize(index + 1);
		// Store the data 
		modules[index] = &(moduleData.second);
      }
      switch(modules.size()) {
		// Easy exit if no surfaces are available
		case 0 : ACTS_VERBOSE("Layer " << layerData.first << " stored no surfaces");
				 break;
		// Easy exit if only a single surface is available 
		case 1 : ACTS_VERBOSE("Layer " << layerData.first << " stored a single surface");
				 break;
		// Form combinations between all hits on both surfaces
		case 2 : combineHits(*(modules[0]), *(modules[1]), combHits);
				 break;
		// Try to combine the hits of every surface with each other
		default : std::vector<FW::SpacePointFinder::CombinedHits> tmpCombHits;
				  for(unsigned int i = 0; i < modules.size(); i++)
					for(unsigned int j = i + 1; j < modules.size(); j++)
					{
						combineHits(*(modules[i]), *(modules[j]), tmpCombHits);
						combHits.insert(combHits.end(), 
										tmpCombHits.begin(), 
										tmpCombHits.end());
				    }	  
	  }
	...
	FW::SpacePointFinder::SpacePointParameters& spaPoPa) const
{
	...
	// Increase the limits. This allows a check if the point is just slightly outside the SDE
    spaPoPa.limitExtended = spaPoPa.limit + m_cfg.stripLengthGapTolerance / spaPoPa.qmag;
    ...
    if(spaPoPa.n == 0.)
		spaPoPa.n = -spaPoPa.t.dot(spaPoPa.qs)
					/ spaPoPa.r.dot(spaPoPa.qs);
	// Check if n is just slightly outside				
    if(fabs(spaPoPa.n) > spaPoPa.limitExtended) return false;
    
	/// The following code considers an overshoot of m and n in the same direction of their SDE. The term "overshoot" represents the amount of m or n outside its regular interval (-1, 1).
	/// It calculates which overshoot is worse. In order to compare both, the overshoot in n is projected onto the first surface by considering the normalized projection of r onto q.
	/// This allows a rescaling of the overshoot. The worse overshoot will be set to +/-1, the parameter with less overshoot will be moved towards 0 by the worse overshoot.
	/// In order to treat both SDEs equally, the rescaling eventually needs to be performed several times. If these shifts allows m and n to be in the limits, the space point can be stored.
    ...
    // Calculate the scaling factor to project lengths of the second SDE on the first SDE
    double secOnFirstScale = spaPoPa.q.dot(spaPoPa.r) / (spaPoPa.qmag * spaPoPa.qmag);
    ...
    if(spaPoPa.m > 1. && spaPoPa.n > 1.)
    {
		...
		double nOvershoot = (spaPoPa.n - 1.) * secOnFirstScale; // Perform projection
		// Resolve worse overshoot
		double biggerOvershoot = (mOvershoot > nOvershoot) ? mOvershoot : nOvershoot;
...
void
FW::SpacePointFinder::storeSpacePoint(
	const Acts::Vector3D& spacePoint,
    ...
    const DetData*&                         detData) const
{
  ...
  for (const auto& layers : allCombHits)
    for (const auto& hits : layers) {
      ...
      /// The same calculation can be repeated for y. Its corresponding parameter will be named n.
      ...
      if(fabs(spaPoPa.m) <= spaPoPa.limit 
         && fabs(spaPoPa.n = -spaPoPa.t.dot(spaPoPa.qs) / spaPoPa.r.dot(spaPoPa.qs)) <= spaPoPa.limit)
	  ...
	  /// If this point is reached then it was not possible to resolve both points such that they are on their SDEs
	  /// The following code treats a possible recovery of points resolved slightly outside of the SDE.
	  /// @note This procedure is an indirect variation of the vertex position.
