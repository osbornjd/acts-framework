...
private:
  ...
  const 
  Acts::Vector2D localCoords(const Acts::PlanarModuleCluster& hit) const;
  
  /// Getter method for the global coordinates of a hit 
  /// @param hit object related to the hit that holds the necessary information
  /// @return vector of the global coordinates of the hit
  const 
  Acts::Vector3D globalCoords(const Acts::PlanarModuleCluster& hit) const;
  ...
  void 
  findOverlappingClusters(const DetData*& detData, 
