    ...
    std::shared_ptr<FW::IWriterT<Acts::ExtrapolationCell<T>>> writer) const
{
  ...
  for (auto& es : ecc.extrapolationSteps) {
    ...
    if (es.material && m_cfg.materialWriter && es.surface) {
      ...
      materialSteps.push_back(Acts::MaterialStep(
          *es.material, es.position, es.surface->geoID().value()));
    }
  }
  ...
  if (m_cfg.materialWriter) {
    ...
    Acts::MaterialTrack mTrack(
        sPosition, sTheta, sPhi, materialSteps, tX0, tL0);
    // call the writer
