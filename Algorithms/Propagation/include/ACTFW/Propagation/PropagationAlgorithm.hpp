...
#define propagateAB(prop, pars, opt, res){ tp_ptr p =           \
      prop->propagate(pars, opt).endParameters;                 \
      if (p) { const auto& pos = p->position();                 \
        ACTS_VERBOSE("Intersection = " << pos.x() << ","        \
                                       << pos.y() << ","        \
                                       << pos.z() )             \
        res.push_back(std::move(p)); } }  

#define propagateSfAB(prop, pars, sf, opt, res){ tp_ptr p =     \
      prop->propagate(pars, sf, opt).endParameters;             \
      if (p) { const auto& pos = p->position();                 \
        ACTS_VERBOSE("Intersection = " << pos.x() << ","        \
                                       << pos.y() << ","        \
                                       << pos.z() )             \
...
public:
  ...
  struct Config
  {
    ...
    Acts::ActsVectorD<5> covariances 
      = Acts::ActsVectorD<5>::Zero();
    /// the correlation terms
    Acts::ActsSymMatrixD<5> correlations 
      = Acts::ActsSymMatrixD<5>::Identity();
  ...
  PropagationAlgorithm(const Config& cnf,
...
private:
  ...
  std::vector< std::unique_ptr<Acts::CylinderSurface > > 
    m_radialSurfaces;
