...
template < typename PropagatorA,  
           typename PropagatorB, 
           typename PropagatorE >
...
template < typename PropagatorA,  
           typename PropagatorB, 
           typename PropagatorE >
PropagationAlgorithm<PropagatorA,PropagatorB,PropagatorE>::PropagationAlgorithm(
    const PropagationAlgorithm<PropagatorA,PropagatorB,PropagatorE>::Config& cfg,
    Acts::Logging::Level loglevel)
  : BareAlgorithm("PropagationAlgorithm", loglevel)
  , m_cfg(cfg)
{
...
template < typename PropagatorA,  
           ...
           typename PropagatorE >
ProcessCode
PropagationAlgorithm<PropagatorA,PropagatorB,PropagatorE>::execute(AlgorithmContext ctx) const
{
  ...
  for (auto& evtx : (*evgen)){
     ...
     for (const auto& particle : sparticles) {
       ...
       if (particle.charge()){
         ...
         Acts::CurvilinearParameters sParameters(std::move(cov), 
                                                      vertex, 
                                                      momentum, 
                                                      charge);
         ...
         if (m_cfg.testMode == kalman) { 
           ...
           if (m_cfg.propagatorA){
             ...
             for (auto& surface : m_radialSurfaces){
               if (m_cfg.cacheCall) propagateCacheAB(m_cfg.propagatorA, 
                                                      *parameters, 
                                                      *(surface.get()),
                                                      cacheA, 
                                                      m_optionsA, 
                                                      tParameters)
               else propagateSfAB(m_cfg.propagatorA, 
                                  *parameters, 
                                  *(surface.get()), 
                                  m_optionsA, 
                                  tParameters); 
