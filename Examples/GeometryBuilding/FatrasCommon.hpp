                ...
                Acts::Logging::Level loglevel = Acts::Logging::INFO)
{
  ...
  auto digitzationAlg = std::make_shared<FW::DigitizationAlgorithm>(
      digConfig, Acts::Logging::VERBOSE);

  FW::StripSpacePointBuilder::Config spfConfig;
  spfConfig.collectionIn  = "FatrasClusters";
  spfConfig.collectionOut = "SCTSpacePoints";
  auto spacepointAlg      = std::make_shared<FW::StripSpacePointBuilder>(
      spfConfig, Acts::Logging::DEBUG);

  // add algorithms to sequencer
  if (sequencer.appendEventAlgorithms(
          {extrapolationAlg, digitzationAlg, spacepointAlg})
      //~ if (sequencer.appendEventAlgorithms({extrapolationAlg,
      //digitzationAlg})
