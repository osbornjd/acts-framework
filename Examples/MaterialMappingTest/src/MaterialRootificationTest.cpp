...
int main() {
    ...
    // set up the writer
    FWRoot::RootMaterialTrackRecReader::Config writerConfig("MaterialWriter",Acts::Logging::VERBOSE);;
    writerConfig.fileName   = "MaterialTrackRecords.root";
    writerConfig.treeName   = "MaterialTrackRecords";
    auto materialTrackRecWriter = std::make_shared<FWRoot::RootMaterialTrackRecWriter>(writerConfig);
    
    // set up the reader
     FWRoot::RootMaterialTrackRecReader::Config readerConfig("MaterialReader",Acts::Logging::VERBOSE);
     readerConfig.fileName   = "/afs/cern.ch/work/j/jhrdinka/ACTS/ACTS/acts-test-fw/MaterialTrackRecords.root";
     readerConfig.treeName   = "MaterialTrackRecords";
     auto materialTrackRecReader = std::make_shared<FWRoot::RootMaterialTrackRecReader>(readerConfig);
    ...
    auto materialRootAlg = std::make_shared<FWE::MaterialRootification>(algorithmConfig);
