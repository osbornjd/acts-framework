...
static Ref_t create_element(LCDD& lcdd, xml_h xml, SensitiveDetector sens)
{
    ...
    //add Extension to Detlement for the RecoGeometry to destinguish cylinder and disc volumes
    ...
    for (xml_coll_t j(xml,_U(layer));j;++j)
    {
      ...
      /*                  Acts::DetExtension* detSensComponent = new Acts::DetExtension(segmentation);
                    ...
                    for (xml_coll_t comp(x_module,_U(module_component));comp;comp++) {
                        ...
                        Volume comp_vol(_toString(comp_num, "component% "),Box(x_component.length(),x_component.width(),x_component.thickness()),lcdd.material(x_component.materialStr()));
           ...
           /*                 Acts::DetExtension* detSensComponent = new Acts::DetExtension(segmentation);
                        ...
                        PlacedVolume placedcomponent = mod_vol.placeVolume(comp_vol,Position(x_component.x(),0.,x_component.z()));
                        placedcomponent.addPhysVolID(_toString(comp_num, "component% "),comp_num);
                        //create detector element
                        DetElement component_det (_toString(comp_num, "component% "),comp_num);
                    ...
                    if(x_module.hasChild(_U(subtraction))) {
                        ...
                        Volume sub_vol("subtraction components",SubtractionSolid(trap_shape,tubs_shape,Transform3D(RotationX(0.5*M_PI))),lcdd.material(x_sub.materialStr()));
                        ...
                        placedSub.addPhysVolID(_toString(comp_num, "component% "),comp_num);
                        //create detector element
                        DetElement sub_det (mod_det,_toString(comp_num, "component% "),comp_num);
                    ...
                    if (x_module.hasChild(_U(tubs))) {
                        ...
                        placedPipe.addPhysVolID(_toString(comp_num, "component% "),comp_num);
                        //create detector element
                        DetElement pipe_det (mod_det,_toString(comp_num, "pipe% "),comp_num);
