...
MyExtrapolator::operator()(const FitMeas_t& m, const TrackParameters& tp) const
{
  ...
  const BoundParameters* pars = new BoundParameters(
      std::make_unique<const ActsSymMatrix<ParValue_t, Acts::NGlobalPars>>(
          *exCell->leadParameters->covariance()),
      exCell->leadParameters->parameters(),
      sf);
...
initExtrapolator(const std::shared_ptr<const TrackingGeometry>& geo)
{
  ...
  propConfig.fieldService
      = std::make_shared<ConstantBField>(0, 0, 0 * Acts::units::_T); // urspruenglicher wert: 2
