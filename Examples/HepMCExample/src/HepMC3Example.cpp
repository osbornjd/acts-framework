...
main(int argc, char* argv[])
{
  ...
  const std::vector<std::shared_ptr<Acts::ProcessVertex>> vertices2 = hepmc3run.event(0)->vertices();
  std::cout << "Number of vertices: " << vertices2.size() << std::endl;
  const std::vector<Acts::ParticleProperties> incoming2 = vertices2[0]->incomingParticles();
  std::cout << "Incoming particles: " << incoming2.size() << std::endl;
  const std::vector<Acts::ParticleProperties> outgoing2 = vertices2[0]->outgoingParticles();
  ...
  std::cout << std::endl;
  std::cout << "Beams: ";
  std::vector<std::shared_ptr<Acts::ParticleProperties>> beams = hepmc3run.event(0)->beams();
  for(auto& beam : beams)
