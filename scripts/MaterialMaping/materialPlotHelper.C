...
#include "../../external/acts-core/Core/include/Acts/Geometry/GeometryID.hpp"
...
void Initialise_info(sinfo& surface_info,
		     const std::map<std::string,std::string>& surface_name, const uint64_t& id, const int& type, const float& pos, const float& range_min, const float& range_max){
  ...
  for(int tag=0;tag<5;tag++){
    Ids[tag].erase(std::remove(Ids[tag].begin(), Ids[tag].end(), ' '), Ids[tag].end());
    Ids[tag].erase(std::remove(Ids[tag].begin(), Ids[tag].end(), '['), Ids[tag].end());
    Ids[tag].erase(std::remove(Ids[tag].begin(), Ids[tag].end(), ']'), Ids[tag].end());
  }

  surface_info.idname ="v"+Ids[0]+"_b"+Ids[1]+"_l"+Ids[2]+"_a"+Ids[3];
  surface_info.type = type;

  if(surface_name.find(surface_id) != surface_name.end()){
    surface_info.name = surface_name.at(surface_id);
  }
  else surface_info.name = "";
