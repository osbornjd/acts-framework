                                         ...
                                         size_t skip)
{
  ...
  if (!m_inputChain){
    ...
    // set the branch
    int branchAddress = m_inputChain->SetBranchAddress<Acts::MaterialTrack>("MaterialTrack", &m_trackRecord);
    ...
    // Full event count
    ACTS_DEBUG("The full chain has " << m_inputChain->GetEntries() << " entries.");
  }
  
  // some screen output
  if (!(m_event%1000))
     ACTS_DEBUG("Mapped " << m_event << " out of " << m_inputChain->GetEntries() << " entries.");
  ...
  ACTS_VERBOSE("Material Track Record read in with phi / theta = " 
    << m_trackRecord->phi() << " / " << m_trackRecord->theta());
