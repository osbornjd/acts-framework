template <class T>
FW::ProcessCode
FW::Root::RootTrackParametersWriter<T>::writeT(const FW::AlgorithmContext&  ctx,
...
FW::Root::RootTrackParametersWriter<T>::RootTrackParametersWriter(
  ...
  : FW::WriterT< std::vector< T > >
    (cfg.collection, "RootTrackParametersWriter", level)
  , m_cfg(cfg)
  ...
  , m_outputTree(nullptr)
{
  ...
  m_outputTree = new TTree(m_cfg.treeName.c_str(), 
                           "TTree from TrackParametersWriter");
  // the brances
  // - the tag                         
