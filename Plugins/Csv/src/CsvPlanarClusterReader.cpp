...
FW::Csv::CsvPlanarClusterReader::read(FW::AlgorithmContext ctx)
{
  ...
  if(detailCsvReader.numPars() < 4 ) {
    ACTS_ERROR("Number of csv parameters in file '" << pathDetails
  ...
  if(truthCsvReader.numPars() < 8 ) {
    ACTS_ERROR("Number of csv parameters in file '" << pathTruth
  ...
  while(hitCsvReader.readLine(hitVal)){
    ...
    m_cfg.tGeometry->visitSurfaces([&hitSurface, &geoID](const Acts::Surface* srf) 
                                   { if(srf->geoID().value() == geoID.value()) hitSurface = srf; });
    if(!hitSurface){ 
      ACTS_ERROR("Could not retrieve the surface with geoID = " << geoID.value() << ". Skipping hit with hit_id = " << hit_id);
      continue;
    }

    // transform global into local position
    Acts::Vector3D pos(std::stof(hitVal[1]), std::stof(hitVal[2]), std::stof(hitVal[3]));
    ...
    while(detailCsvReader.peekLine().size()!=0 
          && std::stoul(detailCsvReader.peekLine()[0]) == std::stoul(hitVal[0])){
      if(detailCsvReader.readLine(detailVal)){
        dCells.push_back(Acts::DigitizationCell(
                                                std::stoul(detailVal[1]),
    ...
    while(truthCsvReader.peekLine().size()!=0
          && std::stoul(truthCsvReader.peekLine()[0]) == std::stoul(hitVal[0])){
      if(truthCsvReader.readLine(truthVal)){
        Acts::Vector3D sPosition(
                                 std::stof(truthVal[2]), 
                                 std::stof(truthVal[3]), 
                                 std::stof(truthVal[4]));
        Acts::Vector3D sMomentum(
                                 std::stof(truthVal[5]),
                                 std::stof(truthVal[6]), 
                                 std::stof(truthVal[7]));
        ...
        ACTS_VERBOSE("particle barcode = " << barcode
    ...
    ACTS_VERBOSE("hit_id = " << hit_id 
                 << " : geoID = " << geoID  
                 << " : globalPos = (" << pos[0] << ", " << pos[1] << ", " << pos[2] << ")" 
                 << " : localPos = (" << local[0] << ", " << local[1] << ")" 
                 << " : nCells = " << dCells.size()
                 << " : nTruthParticles = " << hitParticles.size());

    // create the planar cluster
    Acts::PlanarModuleCluster pCluster(hitSurface->getSharedPtr(), 
  ...
  if (ctx.eventStore.add(m_cfg.outputClusterCollection, std::move(planarClusters))
