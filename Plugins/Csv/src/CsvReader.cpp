...
{ 
  ...
  if(m_nPars == 0) 
    throw std::ios_base::failure("Failed to fetch header from the first line of file '" + m_fileName + "'.");
  ...
  if(read(line)){
    ...
    for (const std::string& csv: csvPars){
      ...
      if(it == m_csvParIDs.end()){ 
        ACTS_ERROR("The csv named '" << csv << "' does not exist according to the .csv header in file '" 
                   << m_fileName << "'.");      
        return false;
      }
...
FW::CsvReader::parName(const size_t id) const
{
  ...
  if(id>m_nPars-1){
    ACTS_ERROR("Maximum input parameter id is " << m_nPars -1 <<". An empty string returned." );
  }
  for (std::map<std::string, int>::const_iterator it 
         = m_csvParIDs.begin(); it != m_csvParIDs.end(); ++it ){
    if (it->second == id) 
      return it->first;
  }
  return name;
