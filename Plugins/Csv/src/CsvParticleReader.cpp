...
FW::Csv::CsvParticleReader::read(FW::AlgorithmContext ctx)
{
  ...
  if(pCsvReader.numPars() < 8 ) {
    ACTS_ERROR("Number of csv parameters in file '" << pathIs
  ...
  while(pCsvReader.readLine(particleVal)){
    ...
    Acts::Vector3D vertex = Acts::Vector3D(std::stof(particleVal[1]), std::stof(particleVal[2]), std::stof(particleVal[3]));
    Acts::Vector3D momentum = Acts::Vector3D(std::stof(particleVal[4]), std::stof(particleVal[5]), std::stof(particleVal[6]));
