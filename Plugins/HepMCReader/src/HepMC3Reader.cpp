...
FW::HepMC3Reader::HepMC3Reader(const std::string& filename, const int version)
{
	...
	switch(version)
	{
		case(2) : reader = new HepMC::ReaderAsciiHepMC2(filename);
				  break;
		case(3) : reader = new HepMC::ReaderAscii(filename);
	}
}
...
FW::HepMC3Reader::buildParticle(HepMC::GenVertex::particles_in_const_iterator it)
{
	// Extract the particles information from the iterator and store it in an ACTS particle object
...
{
	// Extract the information for the member variables of HepMC3Reader::EventHead from HepMC::GenEvent and store these
	...
	if(beam_particles().first && beam_particles().second) evtHead.beamParticles = std::make_pair(beam_particles().first->pid(), beam_particles().second->pid());
