...
namespace Obj {
  ...
  ///
  template <class T>
      class ObjExCellWriter : public WriterT
                              < std::vector<Acts::ExtrapolationCell<T>>
  {
  public:
  ...
  ObjExCellWriter<T>::ObjExCellWriter(const ObjExCellWriter<T>::Config& cfg)
    : FW::IWriterT<Acts::ExtrapolationCell<T>>(), m_cfg(cfg), m_vCounter(0)
  {
