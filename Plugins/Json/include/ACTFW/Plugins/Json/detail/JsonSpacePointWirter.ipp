template <class T>
FWJson::JsonSpacePointWriter<T>::JsonSpacePointWriter(
...
template <class T>
FW::ProcessCode
FWJson::JsonSpacePointWriter<T>::write(
    const FW::DetectorData<geo_id_value, T>& eData)
{
  ...
  for (auto& volumeData : eData) {
    ...
    geo_id_value volumeID
        = volumeData.first.value(Atcts::GeometryID::volume_mask);
    //
